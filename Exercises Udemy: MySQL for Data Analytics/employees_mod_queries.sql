...
#Number of employees per department. Only count where the employees are working currently to avoid double counting
#t_dept_emp, t_departments required

SELECT 
    z2.dept_name, COUNT(z1.emp_no) as number_of_employees
FROM
    t_dept_emp z1
        INNER JOIN
    t_departments z2 ON z1.dept_no = z2.dept_no
WHERE
    z1.to_date = '9999-01-01'
GROUP BY z2.dept_name;


#Visualisation of the number of employees employed into each department since 1990, separated by gender and department.
#Includes the movement of the same employee across departments
